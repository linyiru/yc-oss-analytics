...
export interface Lesson { key: string; title: string; stat: string; statLabel: string; lesson: string; caveat: string; examples: string[] }
  ...
  const lessons: Lesson[] = [
    {
      ...
      lesson: `Repos with a Hacker News post that cracked 100+ points carry a median ${k(med(withHN.map(stars)))} stars — roughly ${(med(withHN.map(stars)) / Math.max(1, med(noHN.map(stars)))).toFixed(1)}× those without. A "Launch HN" or "Show HN" is the single clearest spike on most curves.`,
      caveat: 'Correlation, not causation — strong products are also more likely to reach the HN front page.',
