...
const en: MDContent = {
  ...
  intro: 'Independent analysis of public data on ~158 Y Combinator open-source companies. **Not affiliated with or endorsed by Y Combinator.** Every figure is an approximation with known caveats — we would rather under-claim.',
...
const zhHant: MDContent = {
  ...
  intro: '對約 158 家 Y Combinator 開源公司的公開資料所做的獨立分析。**與 Y Combinator 無任何關聯,也未經其背書。** 每個數字都是帶著已知限制的近似值——我們寧可說得保守一點。',
  ...
  defs: [
    ...
    ['Star 成長曲線', '每日累積 star,從 GH Archive 的 WatchEvents(透過 BigQuery)重建,以每個 repo 穩定的數字 id 為鍵——所以改名永遠不會弄斷歷史。158 個 repo 裡有 156 個完整;少數源自 2015 年以前的會顯示一個錨定的基線,並標記為「partial」。'],
...
const zhHans: MDContent = {
  ...
  intro: '对约 158 家 Y Combinator 开源公司的公开数据所做的独立分析。**与 Y Combinator 无任何关联,也未经其背书。** 每个数字都是带着已知局限的近似值——我们宁可说得保守一点。',
  ...
  defs: [
    ...
    ['Star 成长曲线', '每日累积 star,从 GH Archive 的 WatchEvents(通过 BigQuery)重建,以每个 repo 稳定的数字 id 为键——所以改名永远不会弄断历史。158 个 repo 里有 156 个完整;少数源自 2015 年以前的会显示一个锚定的基线,并标记为「partial」。'],
