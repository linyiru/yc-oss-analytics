...
Input:  data/bq_early_network.csv  (repo_id, first100, first100_net, first1000, first1000_net)
...
Output: data/repos/<slug>.json  engagement.early_network = {
          first100, first100_net_pct, first1000, first1000_net_pct }

The percentages are the share of a repo's earliest stargazers who also star >=2 YC
open-source repos — a de-identified read on YC-network distribution. No logins are stored.
"""
...
def main():
    ...
    for f in glob.glob(os.path.join(ROOT, "data", "repos", "*.json")):
        ...
        if not r:
            continue
        f100, f1k = int(r["first100"]), int(r["first1000"])
